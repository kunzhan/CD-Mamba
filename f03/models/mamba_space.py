    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                A_b = -torch.exp(self.A_b_log.float())
                xzd = torch.chunk(xz, chunks=2, dim=1)
                xz1 = torch.cat((xzd[0], xzd[1].flip([-1])), dim=1)  
                xz2 = torch.cat((xzd[0].flip([-1]), xzd[1]), dim=1)  
                ...
                xz1 = torch.chunk(out_c, chunks=2, dim=1)
                xz1 = torch.cat((xz1[0], xz1[1].flip([-1])), dim=1)  
                xz2 = torch.chunk(out_d, chunks=2, dim=1)
                xz2 = torch.cat((xz2[0].flip([-1]), xz2[1]), dim=1)  
                # F.linear(rearrange(out_z, "b d l -> b l d"), out_proj_weight, out_proj_bias)
                if not self.if_devide_out:
                    out = F.linear(rearrange(out + out_b.flip([-1]) + xz1 + xz2, "b d l -> b l d"), self.out_proj.weight, self.out_proj.bias)
                else:
