...
class CDnetV1_MODEL(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=1, aux=True):
        self.inplanes = 64
        self.aux = aux		
        # block=Bottleneck
        # layers=[3, 4, 6, 3]
    ...
    def forward(self, x):
        ...
        # return {'out':score3,'out1':predict1,'out2':predict2,'out3':predict3}# score3, predict1, predict2, predict3
        return torch.sigmoid(score3)	
...
if __name__ == '__main__':
    model = CDnetV1_MODEL().cpu()
    x = torch.randn(2, 4, 384, 384).cpu()
    ...
    fps = 10 / total_time  # 或者 fps = 1.0 / avg_time_per_frame
